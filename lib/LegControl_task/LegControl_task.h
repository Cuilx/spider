...
public:
	...
	void body_position_cal(float xValue, float yValue, float zValue); // 控制机体位置平移(x身体左右平移)，(y身体前后平移), (z表示上下平移)
	void body_angle_cal(float xValue, float yValue, float zValue);	  // 控制机体角度(平转) x(左，右平转), y(头高和低), z(身体左右倾斜) 参数由对象body_angle传递
	...
	void waveGait();
	void moveLeg(int legIndex, int joint1Angle, int joint2Angle, int joint3Angle);
};
...
void Hexapod::Init(uint8_t mode)
{
	...
	if (mode == 0)
	{
		...
		for (int i = 0; i < 6; i++) // 遍历六条腿进行初始化
		{
			...
			arm.servoMove(3 * i + 3, 95, 1000);	 // 初始化第三关节角度为140度
		}
		arm.servoMove(19, 150, 1000);
		arm.servoMove(20, 150, 1000);
		arm.servoMove(21, 150, 1000);
...
void Hexapod::velocity_cal(int RXValue, int RYValue, int LXValue)
{
	...
	// 直接计算并检查接近零的值
	// velocity.Vx = (std::abs(RXValue) < 0.0001f) ? 0 : velocity_fof[0].cal(0.4f * RXValue);	 // 如果RXValue的绝对值小于0.0001，则Vx为0，否则为0.4f * RXValue
	// velocity.Vy = (std::abs(RYValue) < 0.0001f) ? 0 : velocity_fof[1].cal(0.4f * RYValue);	 // 如果RYValue的绝对值小于0.0001，则Vy为0，否则为0.4f * RYValue
	// velocity.omega = (std::abs(LXValue) < 0.0001f) ? 0 : velocity_fof[2].cal(0.4f * LXValue); // 如果LXValue的绝对值小于0.0001，则omega为0，否则为0.4f * LXValue

	velocity.Vx = (std::abs(RXValue) < 0.0001f) ? 0 : 0.4f * RXValue;
	velocity.Vy = (std::abs(RYValue) < 0.0001f) ? 0 : 0.4f * RYValue;
	velocity.omega = (std::abs(LXValue) < 0.0001f) ? 0 : 0.4f * LXValue;
	gait_prg.set_velocity(velocity); // 设置步态程序的速度
}

void Hexapod::body_position_cal(float xValue, float yValue, float zValue)
{
...
void Hexapod::body_angle_cal(float xValue, float yValue, float zValue)
{
...
void Hexapod::move(uint32_t round_time)
{
	...
	for (int i = 0; i < 6; i++) // 遍历六条腿
	{
		...
		if (this->mode == HEXAPOD_MOVE)
		{
			...
			arm.servoMove(servo_base_index + 1, (reverse_angle_1 ? -1 : 1) * theta_temp.angle[1] / PI * 180 + 110, round_time);
		...
		else
		{
			...
			arm.servoMove(servo_base_index + 1, (reverse_angle_1 ? -1 : 1) * theta_temp.angle[1] / PI * 180 + 110, round_time);//105
			arm.servoMove(servo_base_index + 2, (reverse_angle_2 ? -1 : 1) * theta_temp.angle[2] / PI * 180 + 190, round_time);//195
		}
	}
}
...
float UserMpu::mpu_Get_angle(char x)
{
	if (mpu.dmpGetCurrentFIFOPacket(fifoBuffer))
	{
		...
		// Serial.print("\t");
		// Serial.print(ypr[1] * 180 / M_PI);  // Y轴角度1
		// Serial.print("\t");
		// Serial.println(ypr[2] * 180 / M_PI);  //Z轴角度33
		if (x == 'x')
...
// 定义每条腿的初始角度
int joint1Angles[6] = {150, 150, 150, 150, 150, 150}; // 中位
int joint2Angles[6] = {115, 115, 115, 115, 115, 115}; // 中位
int joint3Angles[6] = {105, 105, 105, 105, 105, 105}; // 中位
#define width1 30
#define width2 55
#define width3 50
#define delayTime 200

void  Hexapod::moveLeg(int legIndex, int joint1Angle, int joint2Angle, int joint3Angle)
{

	arm.servoMove(legIndex * 3 + 1, joint1Angle, delayTime);
	arm.servoMove(legIndex * 3 + 2, joint2Angle, delayTime);
	arm.servoMove(legIndex * 3 + 3, joint3Angle, delayTime);
}
void  Hexapod::waveGait()
{
	int order[] = {0, 1, 2, 3, 4, 5}; // 恢复正常的移动顺序
	for (int j = 0; j < 6; j++)
	{
		int i = order[j];
		int direction = (i < 3) ? -1 : 1; // 当i<3时，direction为-1，反转逻辑
		// 提升和前移当前腿
		joint1Angles[i] += width1 * direction; // 向前
		joint2Angles[i] -= width2;			   // 提升
		joint3Angles[i] -= width3;			   // 提升
		moveLeg(i, joint1Angles[i], joint2Angles[i], joint3Angles[i]);
		delay(delayTime + 50);

		// 放下当前腿
		joint2Angles[i] += width2; // 放下
		joint3Angles[i] += width3; // 放下
		moveLeg(i, joint1Angles[i], joint2Angles[i], joint3Angles[i]);
		delay(delayTime + 50);
		// 后移当前腿
		joint1Angles[i] -= width1 * direction; // 向后
		moveLeg(i, joint1Angles[i], joint2Angles[i], joint3Angles[i]);
		delay(delayTime + 50);
	}
}
