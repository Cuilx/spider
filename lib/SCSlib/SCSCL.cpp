...
int SCSCL::SetID(u8 targID, u8 newID)
{
	unLockEprom(targID);
	u8 bBuf[1];
	bBuf[0]=newID;
	// Host2SCS(bBuf+0, bBuf+1, newID);
	 int ret=genWrite(targID, SCSCL_ID, bBuf, 1);
	 LockEprom(newID);
	 return ret;
}
int SCSCL::SetID(u8 newID)
{
	unLockEprom(0XFE);
	u8 bBuf[1];
	bBuf[0]=newID;
	// Host2SCS(bBuf+0, bBuf+1, newID);
	 int ret=genWrite(0XFE, SCSCL_ID, bBuf, 1);
	 LockEprom(newID);
	 return ret;
}
