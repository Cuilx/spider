...
void SCSCL::servoMove(uint8_t id, int16_t pos, uint16_t time )
{
  int angle =map((pos+servoDev[id-1]),0,300,0,1023);
  WritePos(id, angle, time, 0);
  Serial.printf("servo%d move to %d\n",id,pos+servoDev[id-1]);
}